...
namespace OpenVISDIVE
{
    ...
    public class DataSyncUI : MonoBehaviour
    {
        ...
        {
            dataSync.StartLoadingScenario += StartLoadingMode;
            dataSync.StartLoadingRepresentation += StartLoadingMode;
            dataSync.ScenarioLoaded += StopLoadingMode;
            dataSync.RepresentationLoaded += StopLoadingMode;
        }
        ...
        protected virtual void Update()
        {
            if (dataSync.Busy && progressImage)
            {
                ...
                if (dataSync.Progress < Mathf.Epsilon)
                {
                    progressImage.fillAmount = 0.1f;
                    progressImage.transform.Rotate(Vector3.back, Time.deltaTime * 100f);
                }
                else
                {
                    progressImage.transform.localRotation = Quaternion.identity;
